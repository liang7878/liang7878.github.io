...
description: "一提到限流算法，大家肯定就是要聊时间窗口、漏斗桶、令牌桶之类的，今天跟着Azure的文档来看看限流器在生产环境怎么搭建和使用。"
tags:
  - 技术杂谈
  - Azure
  - 阅读
...
> **速记**
> - 速率限制不是简单拒绝流量，而是把需求端洪峰与服务端能力解耦，避免重试风暴。
> - 常见实现：可靠队列 + Worker 控制节奏；多进程共享容量时用分布式锁分配配额。
> - 限流仍要配合错误重试、配额划分和跨团队协调，才能落地在复杂业务中。
...
![耐久消息服务架构图](durable_msging_service.png)
...
![限流策略处理流程图](rate_limited_flow.png)
...
![Azure Blob 存储架构图](azure_blob_storate.png)
