...
> **速记**
> - Shared-Nothing + Paxos 复制组是 OB 的基础，RootService 负责元数据和负载均衡。
> - 存储层采用 LSM Tree，行/块缓存兼顾 OLTP 与 OLAP，配合 WAL 与快照保证恢复。
> - 多租户通过 Resource Unit/Pool 隔离资源，高可用依赖日志复制、备份与两地三中心部署。
...
![OceanBase 发展时间线图](ob_timeline.png)
...
![OceanBase 三层架构示意图](ob_arch.png)

两地三中心如下图：

![OceanBase 2R3C 架构示意图](2r3c.png)

OB的源码文件目录结构如下：

![OceanBase 目录结构视图](folders.png)
...
![Observer 子系统组成图](observer_subsystem.png)
...
![OceanBase 存储引擎组件关系图](storage.png)
...
![OceanBase 启动流程示意图](bootstrap.png)
...
![平面行存储格式图](plane_row.png)

![稀疏平面行存储格式图](sparse_plane_row.png)
...
![存储组件关系示意图](store_component_relation.png)
...
![SQL 引擎架构组成图](sql_engine_arch.png)
...
![并行执行框架组成图](parallel_execution.png)
...
![会话信息结构示意图（一）](session_info_1.png)
...
![会话信息结构示意图（二）](session_info_2.png)

会话中有关事务最核心的部分是trans*desc*属性，它是一个ObTransDesc类的对象，它描述了会话中活跃事务的详情，其结构如上图所示：
...
18. `trx_idle_timeout_`：事务的超时时长，结合`last_end_stmt_ts_`字段的值就能判断一个事务当前是否已经闲置过长时间，如果发现此类情况则强制回滚事务节省系统资源。
